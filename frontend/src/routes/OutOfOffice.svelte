<script>
    ...
    $: approvedRequests = requests.filter(request => request.status.toLowerCase() === 'approved');
    ...
    // apply sorting to the approved requests
    $: sortedRequests = [...approvedRequests].sort((a, b) => {
  ...
  <div class="flex min-h-screen w-full flex-col">
    ...
    <main class="bg-muted/40 flex flex-1 flex-col gap-4 p-4 md:gap-8 md:p-8">
      ...
      <Card.Root>
        <Card.Header>
          <Card.Title>Out Of Office</Card.Title>
          <Card.Description>
            See what employees are not available for work.
          </Card.Description>
        </Card.Header>
        <Card.Content>
          <Table.Root>
        ...
        <Card.Footer class="flex justify-between items-center">
          <div class="text-muted-foreground text-xs">
            Showing <strong>{startIndex + 1}-{endIndex}</strong> of <strong>{sortedRequests.length}</strong> employees
          </div>
